...
_PyGC_InitState(GCState *gcstate)
{
    ...
    gcstate->old[0].threshold = 10;
    gcstate->old[1].threshold = 0;
    gcstate->work_to_do = -5000;
...
Py_ssize_t _PyGC_Collect(PyThreadState *tstate, int generation);
...
/*[clinic end generated code: output=b697e633043233c7 input=40720128b682d879]*/
{
    ...
    return _PyGC_Collect(tstate, generation);
}
...
gc_get_referrers(PyObject *self, PyObject *args)
{
    ...
    GCState *gcstate = get_gc_state();
    if (!(gc_referrers_for(args, &gcstate->young.head, result))) {
        goto error;
    }
    if (!(gc_referrers_for(args, &gcstate->old[gcstate->visited_space].head, result))) {
        goto error;
    }
    if (!(gc_referrers_for(args, &gcstate->old[gcstate->visited_space^1].head, result))) {
        goto error;
    }
    return result;
    error:
...
/*[clinic end generated code: output=48b35fea4ba6cb0e input=ef7da9df9806754c]*/
{
    ...
    for (i = 0; i < NUM_GENERATIONS; i++) {
        if (append_objects(result, GEN_HEAD(gcstate, (int)i))) {
            goto error;
        }
    }
    return result;
...
Py_ssize_t
_PyGC_Collect(PyThreadState *tstate, int generation)
{
    GCState *gcstate = &tstate->interp->gc;
...
PyGC_Collect(void)
{
    return _PyGC_Collect(_PyThreadState_GET(), 2);
}
...
_PyGC_CollectNoFail(PyThreadState *tstate)
{
    /* Ideally, this function is only called on interpreter shutdown,
       ...
       */
    _PyGC_Collect(_PyThreadState_GET(), 2);
}
